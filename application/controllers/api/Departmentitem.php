...
class Departmentitem extends REST_Controller {
    ...
    */
     ...
     * @return Response
    */
	public function index_get($id = 0,$userid=0,$isactive=0,$serachkewords='')
	{
	   
	    $addtype=2;
	    $likes=1;
        if(!empty($id)){
            $data = $this->db->get_where("bookpdf", ['id' => $id])->row_array();
        }elseif($serachkewords!=''){
...
$this->db->join('likeandbookmarks', 'bookpdf.id = likeandbookmarks.ebook_id','left');
$this->db->like('bookpdf.title',$serachkewords);
...
$this->db->from('bookpdf');
$this->db->join('likeandbookmarks', 'bookpdf.id = likeandbookmarks.ebook_id','left');
$this->db->where('bookpdf.topic_id',$isactive);
$this->db->group_by('bookpdf.id');
$this->db->order_by('numb',"DESC");
  $data= $this->db->get()->result();
        }
